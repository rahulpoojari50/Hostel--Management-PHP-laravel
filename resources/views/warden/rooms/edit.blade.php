...
@section('title', 'Edit Room - ' . $hostel->name)

@section('content')
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Edit Room – {{ $hostel->name }}</h1>
        <div>
    ...
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0">Edit Room: {{ $room->room_number }}</h5>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('warden.hostels.rooms.update', [$hostel, $room]) }}">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('current_occupants') is-invalid @enderror" 
                                   id="current_occupants" name="current_occupants" value="{{ old('current_occupants', $room->current_occupants) }}" min="0" required>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('max_occupants') is-invalid @enderror" 
                                   id="max_occupants" name="max_occupants" value="{{ old('max_occupants', $room->max_occupants) }}" min="1" required>
                            @error('max_occupants')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
